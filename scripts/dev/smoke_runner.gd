...
# Headless smoke runner — plays KM27 badly but completely, with no window and
# no human, to catch script errors before a person ever rolls the broken week.
#
...
#
# Deliberately NEVER calls SaveManager.save_game() — a headless pass must not
# clobber a real save on a machine that has one.
...
func _ready() -> void:
	...
	for i in range(seeds):
		...
		if i == 0:
			first_trace = r["trace"]
		print("[smoke] seed %-6d  %s" % [seed_value, _summary_line(r)])
		if not r["ok"]:
			failures += 1
			print("[smoke]   FAIL: %s" % r["fail"])
	...
	if replay["trace"] != first_trace:
		failures += 1
		print("[smoke] FAIL: determinism — seed %d produced a different trace on replay." % start_seed)
...
func _print_trace_diff(a: String, b: String) -> void:
	var la: PackedStringArray = a.split("\n")
	var lb: PackedStringArray = b.split("\n")
	for i in range(mini(la.size(), lb.size())):
		if la[i] != lb[i]:
			print("[smoke]   first divergence at line %d:" % i)
			print("[smoke]     run A: %s" % la[i])
			print("[smoke]     run B: %s" % lb[i])
			return
	print("[smoke]   traces differ in length: %d vs %d lines" % [la.size(), lb.size()])
